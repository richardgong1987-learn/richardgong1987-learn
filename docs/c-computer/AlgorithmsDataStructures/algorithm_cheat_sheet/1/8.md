...
![img](https://gblobscdn.gitbook.com/assets%2F-MgaO_np8dKO0rzhRKXx%2Fsync%2Ff2f977954117e3643323d2eac4c3668f502835d1.png?alt=media)
...
![img](https://gblobscdn.gitbook.com/assets%2F-MgaO_np8dKO0rzhRKXx%2Fsync%2F298b4971971d6e850923f64ab74792b86aa5c668.png?alt=media)
...
显然 `i = 0` 时 `i-1` 是不合法的索引，这是因为我们没有对 `i` 的 base case 进行处理。可以这样处理：
...
这个疑问很正确，因为我们前文 [动态规划答疑篇](https://labuladong.gitbook.io/algo/mu-lu-ye-2/mu-lu-ye/zui-you-zi-jie-gou) 有介绍 `dp` 数组的遍历顺序是怎么确定的，主要是根据 base case，以 base case 为起点，逐步向结果靠近。
...
你买股票，初始的「状态」是什么？应该是从第 0 天开始，因为还没有进行买卖，所以最大交易次数 `k` 应该是 `max_k`，而随着对状态的穷举，交易次数上限 `k` 应该不断减少，这样一想，`k = max_k, k--` 的方式是比较合乎我们理解的。
